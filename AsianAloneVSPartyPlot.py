...
plt.scatter(votes['RHI425214'], votes['Trump'], s=8, c='r', label='X-Axis: % Asian people vs. Y-Axis: % Votes for Rep. in 2016')
plt.scatter(votes['RHI425214'], votes['Clinton'], s=8, c='b', label='X-Axis: % Asian people vs. Y-Axis: % Votes for Dem. in 2016') 
plt.legend()
X = votes.loc[:,'RHI425214'].values
Y = votes.loc[:,'Trump'].values
r = CorrelationCoeff(X,Y)
print('Correlation Coefficient for Asian people Vs Republicans  = ' + str(r))

Z = votes.loc[:,'RHI425214'].values
W = votes.loc[:,'Clinton'].values
s = CorrelationCoeff(Z,W)
print('Correlation Coefficient for Asian people Vs Democrats  = ' + str(s))
