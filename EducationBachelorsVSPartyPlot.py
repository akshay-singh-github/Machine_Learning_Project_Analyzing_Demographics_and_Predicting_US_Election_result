...
plt.scatter(votes['Edu_batchelors'], votes['Trump'], s=8, c='r', label='X-Axis: % Bachelors Deg. Grads vs. Y-Axis: % Votes for Rep. in 2016')
plt.scatter(votes['Edu_batchelors'], votes['Clinton'], s=8, c='b', label='X-Axis: % Bachelors Deg. Grads vs. Y-Axis: % Votes for Dem. in 2016') 
plt.legend()
X = votes.loc[:,'Edu_batchelors'].values
Y = votes.loc[:,'Trump'].values
r = CorrelationCoeff(X,Y)
print('Correlation Coefficient for Edu_batchelors Vs Republicans  = ' + str(r))

Z = votes.loc[:,'Edu_batchelors'].values
W = votes.loc[:,'Clinton'].values
s = CorrelationCoeff(Z,W)
print('Correlation Coefficient for Edu_batchelors Vs Democrats  = ' + str(s))
