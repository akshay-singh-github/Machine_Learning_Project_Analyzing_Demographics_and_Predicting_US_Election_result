...
plt.scatter(votes['Edu_highschool'], votes['Trump'], s=8, c='r', label='X-Axis: % High School Grads vs. Y-Axis: % Votes for Rep. in 2016')
plt.scatter(votes['Edu_highschool'], votes['Clinton'], s=8, c='b', label='X-Axis: % High School Grads vs. Y-Axis: % Votes for Dem. in 2016') 
plt.legend()
X = votes.loc[:,'Edu_highschool'].values
Y = votes.loc[:,'Trump'].values
r = CorrelationCoeff(X,Y)
print('Correlation Coefficient for Edu_highschool Vs Republicans  = ' + str(r))

Z = votes.loc[:,'Edu_highschool'].values
W = votes.loc[:,'Clinton'].values
s = CorrelationCoeff(Z,W)
print('Correlation Coefficient for Edu_highschool Vs Democrats  = ' + str(s))
