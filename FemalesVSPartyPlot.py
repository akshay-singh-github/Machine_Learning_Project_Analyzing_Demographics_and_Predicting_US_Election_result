...
plt.scatter(votes['SEX255214'], votes['Trump'], s=8, c='r', label='X-Axis: % Females vs. Y-Axis: % Votes for Rep. in 2016')
plt.scatter(votes['SEX255214'], votes['Clinton'], s=8, c='b', label='X-Axis: % Females vs. Y-Axis: % Votes for Dem. in 2016') 
plt.legend()
X = votes.loc[:,'SEX255214'].values
Y = votes.loc[:,'Trump'].values
r = CorrelationCoeff(X,Y)
print('Correlation Coefficient for Females Vs Republicans  = ' + str(r))

Z = votes.loc[:,'SEX255214'].values
W = votes.loc[:,'Clinton'].values
s = CorrelationCoeff(Z,W)
print('Correlation Coefficient for Females Vs Democrats  = ' + str(s))
