...
plt.scatter(votes['Hispanic'], votes['Trump'], s=8, c='r', label='X-Axis: % Hispanic people vs. Y-Axis: % Votes for Rep. in 2016')
plt.scatter(votes['Hispanic'], votes['Clinton'], s=8, c='b', label='X-Axis: % Hispanic people vs. Y-Axis: % Votes for Dem. in 2016') 
plt.legend()
X = votes.loc[:,'Hispanic'].values
Y = votes.loc[:,'Trump'].values
r = CorrelationCoeff(X,Y)
print('Correlation Coefficient for Hispanic people Vs Republicans  = ' + str(r))

Z = votes.loc[:,'Hispanic'].values
W = votes.loc[:,'Clinton'].values
s = CorrelationCoeff(Z,W)
print('Correlation Coefficient for Hispanic people Vs Democrats  = ' + str(s))
