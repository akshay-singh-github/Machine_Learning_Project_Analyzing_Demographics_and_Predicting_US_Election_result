...
plt.scatter(votes['Income'], votes['Trump'], s=8, c='r', label='X-Axis:  Income vs. Y-Axis: % Votes for Rep. in 2016')
plt.scatter(votes['Income'], votes['Clinton'], s=8, c='b', label='X-Axis:  Income vs. Y-Axis: % Votes for Dem. in 2016') 
plt.legend()
X = votes.loc[:,'Income'].values
Y = votes.loc[:,'Trump'].values
r = CorrelationCoeff(X,Y)
print('Correlation Coefficient for Income Vs Republicans  = ' + str(r))

Z = votes.loc[:,'Income'].values
W = votes.loc[:,'Clinton'].values
s = CorrelationCoeff(Z,W)
print('Correlation Coefficient for Income Vs Democrats  = ' + str(s))
