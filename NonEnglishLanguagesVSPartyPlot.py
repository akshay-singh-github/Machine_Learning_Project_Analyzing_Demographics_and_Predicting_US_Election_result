...
plt.scatter(votes['NonEnglish'], votes['Trump'], s=8, c='r', label='X-Axis: % Non - English Lang. vs. Y-Axis: % Votes for Rep. in 2016')
plt.scatter(votes['NonEnglish'], votes['Clinton'], s=8, c='b', label='X-Axis: % Non - English Lang. vs. Y-Axis: % Votes for Dem. in 2016') 
plt.legend()
X = votes.loc[:,'NonEnglish'].values
Y = votes.loc[:,'Trump'].values
r = CorrelationCoeff(X,Y)
print('Correlation Coefficient for Non - English Lang. Vs Republicans  = ' + str(r))

Z = votes.loc[:,'NonEnglish'].values
W = votes.loc[:,'Clinton'].values
s = CorrelationCoeff(Z,W)
print('Correlation Coefficient for Non - English Lang. Vs Democrats  = ' + str(s))
