...
plt.scatter(votes['Poverty'], votes['Trump'], s=8, c='r', label='X-Axis: % Poverty vs. Y-Axis: % Votes for Rep. in 2016')
plt.scatter(votes['Poverty'], votes['Clinton'], s=8, c='b', label='X-Axis: % Poverty vs. Y-Axis: % Votes for Dem. in 2016') 
plt.legend()
X = votes.loc[:,'Poverty'].values
Y = votes.loc[:,'Trump'].values
r = CorrelationCoeff(X,Y)
print('Correlation Coefficient for Poverty Vs Republicans  = ' + str(r))

Z = votes.loc[:,'Poverty'].values
W = votes.loc[:,'Clinton'].values
s = CorrelationCoeff(Z,W)
print('Correlation Coefficient for Poverty Vs Democrats  = ' + str(s))
