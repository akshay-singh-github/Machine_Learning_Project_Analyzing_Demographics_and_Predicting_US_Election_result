...
plt.scatter(votes['VET605213'], votes['Trump'], s=8, c='r', label='X-Axis: % Veteran people vs. Y-Axis: % Votes for Rep. in 2016')
plt.scatter(votes['VET605213'], votes['Clinton'], s=8, c='b', label='X-Axis: % Veteran people vs. Y-Axis: % Votes for Dem. in 2016') 
plt.legend()
X = votes.loc[:,'VET605213'].values
Y = votes.loc[:,'Trump'].values
r = CorrelationCoeff(X,Y)
print('Correlation Coefficient for Veteran people Vs Republicans  = ' + str(r))

Z = votes.loc[:,'VET605213'].values
W = votes.loc[:,'Clinton'].values
s = CorrelationCoeff(Z,W)
print('Correlation Coefficient for Veteran people Vs Democrats  = ' + str(s))
