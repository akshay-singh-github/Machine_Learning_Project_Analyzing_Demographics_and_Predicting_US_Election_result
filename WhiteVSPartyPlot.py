...
plt.scatter(votes['White'], votes['Trump'], s=8, c='r', label='X-Axis: % White people vs. Y-Axis: % Votes for Rep. in 2016')
plt.scatter(votes['White'], votes['Clinton'], s=8, c='b', label='X-Axis: % White people vs. Y-Axis: % Votes for Dem. in 2016') 
plt.legend()
X = votes.loc[:,'White'].values
Y = votes.loc[:,'Trump'].values
r = CorrelationCoeff(X,Y)
print('Correlation Coefficient for White people Vs Republicans  = ' + str(r))

Z = votes.loc[:,'White'].values
W = votes.loc[:,'Clinton'].values
s = CorrelationCoeff(Z,W)
print('Correlation Coefficient for White people Vs Democrats  = ' + str(s))
